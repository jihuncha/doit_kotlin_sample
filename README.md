...
~~~
  ...
  * 파이어베이스 클라우드 메시징 (FCM)
    * 서버에서 특정 상황이나 데이터가 발생할 떄 앱에 알림을 전달하는 기능 (푸시)
  
  * 생각할 수 있는 문제
    * 앱은 서버가 언제 데이터를 보낼지 모른다. -> 앱에서 서비스 컴포넌트로 지속적으로 네트워크와 연결해야함
    * 문제는 백그라운드 제약 사항때문에 앱이 포그라운드 상황이 아닌 이상 지속적으로 연결 불가능
    * 또한 효율성이 부족한 점
  
  * 클라우드 메시징
    * 서버 데이터를 앱에 직접 전달하는 방식이 아닌 FCM 서버를 거쳐서 전달함 -> 앱이 포그라운드 상황이 아니여도 데이터를 받을 수 있다!!
    * 단계는 1. FCM앱을 식별하는 토큰을 받아옴 2. 서버의 데이터를 앱에 전달한다
    
  * 데이터와 별도로 알림 정보를 두어서 알림구성정보와 메시지 정보를 분류할 수 있다.  
